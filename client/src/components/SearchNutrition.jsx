  ...
  useEffect(() => {
    const getAllData = async () => {
      try {
        ...
        const restrictCategories = ["breakfast", "mains", "snacks", "dinners"];

        // Here we fetch the all videos and if tab === nutrition then simply we fetch those items where category is nutrition from them. And if category is equal to any of the restrictCategories then we do that category as searchItem. Beacuse we are fetching perticular category videos from all videos.
        const res = await fetch(
          `/api/search/allcontent?tab=${tab}&category=${
            tab === "nutrition" ? tab : category
          }&searchItem=${
            restrictCategories.includes(category) ? category : searchItem
          }&page=${currentPage}`
        );
